...
import 'package:lumio/features/player/m3u_episode_screen.dart';
...
class _M3uSeriesScreenState extends State<M3uSeriesScreen> {
  late final M3uRepository? _repository;
  ...
  void initState() {
    super.initState();
    _repository = AppState.m3uRepository;
  ...
  Future<void> _loadSeriesDetails() async {
    if (_repository == null) {
      setState(() {
        error = 'M3U repository not initialized';
        isLoading = false;
      });
      return;
    }
    ...
    return Container(
      ...
      child: InkWell(
        ...
        onTap: () async {
          ...
          var m3uItem = await repo.getM3uItemByUrl(url: episode.url);
